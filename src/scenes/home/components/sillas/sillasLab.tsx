...
export function SillasLab() {
  ...
  useEffect(() => {
    if (!seatRef.current || !baseRef.current) return;
  ...
  return (
    <group>
      <instancedMesh
        ref={seatRef}
        args={[nodes.Seat.geometry, materials['Dark metal'], seatInstances.length]}
      />
      <instancedMesh
        ref={baseRef}
        args={[nodes.Base.geometry, materials['Frozen white metal'], baseInstances.length]}
      />
    </group>
  );
}
